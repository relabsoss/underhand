...
class ChildListMutation extends Action {
  constructor(origin, delta) {
    ...
    for (let i = 0; i < origin.addedNodes.length; i++) {
      ...
      this.added.push(node.nodeType === Node.TEXT_NODE ? node.data : node.outerHTML);
    }

    for (let i = 0; i < origin.removedNodes.length; i++) {
      const node = origin.removedNodes.item(i);

      this.removed.push(node.nodeType === Node.TEXT_NODE ? '#text' : node.tagName.toLowerCase());
    }
  }
